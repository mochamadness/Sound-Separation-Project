...
def extract_feature(file_name):
    try:
        # Resample to 22050 for compatibility with librosa features
        ...
        contrast = np.mean(librosa.feature.spectral_contrast(S=stft, sr=sample_rate, n_bands=6, fmin=50).T, axis=0)
...
def predict_gender(file_path):
    ...
    if np.max(np.abs(y)) < 0.05:
        return "⚠️ Âm lượng quá nhỏ, không đủ để dự đoán."
    features = extract_feature(file_path)
    if features is not None:
        features = features.reshape(1, -1)
        expected_features = 40 + 12 + 128 + 7 + 6  # MFCC + Chroma + Mel + Contrast + Tonnetz
        if features.shape[1] != expected_features:
            return f"⚠️ Số đặc trưng không phù hợp: {features.shape[1]}/{expected_features}"
        else:
            model = st.session_state.gender_model
            male_prob = model.predict(features)[0][0]
            female_prob = 1 - male_prob
            if abs(male_prob - female_prob) < 0.1:
                return f"⚠️ Giọng không rõ ràng (khó phân biệt)\n\n🔵 Nam: {male_prob*100:.2f}% | 🔴 Nữ: {female_prob*100:.2f}%"
